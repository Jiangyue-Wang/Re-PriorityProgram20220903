...
##remove all new
library(stringr)
newid<-SPList[str_detect(SPList$sp.file,"20220903"),"rijid"]
newname<-SPList[str_detect(SPList$sp.file,"20220903"),"scientific.name"]

SPList$propnew <- (SPList$target-SPList$InPA)*0.95/SPList$OutPA
...
target_5km_30_sp<-data.frame(feature=SPList$scientific.name,sense=">=",target=SPList$propnew,type="relative")
...
feature_5km_30_sp <- feature_5km_30_sp[!feature_5km_30_sp$id%in%newid$rijid,]
target_5km_30_sp <- target_5km_30_sp[!target_5km_30_sp$feature%in%newname$scientific.name,]
rij_5km_30_sp <- rij_5km_30_sp[!rij_5km_30_sp$species%in%newid$rijid,]
