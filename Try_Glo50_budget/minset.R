...
spunr <- read.csv("Output/SPlist_all_glo30_0.csv")
spunr <- spunr[spunr$Reached==0,]$rijid
rij_5km_30_sp <- rij_5km_30_sp[!rij_5km_30_sp$species%in%spunr,]
feature_5km_30_sp <- feature_5km_30_sp[!feature_5km_30_sp$id%in%spunr,]
target_5km_30_sp<-target_5km_30_sp[target_5km_30_sp$feature%in%feature_5km_30_sp$name,]

sp95<-read.csv("Output/SPlist_all_glo30_0.csv")
library(dplyr)
colnames(sp95)[2]<-"feature"
sp95$pass_floor<-sp95$pass_floor/sp95$OutPA
target_5km_30_sp<-left_join(target_5km_30_sp,sp95[,c("feature","pass_floor")],by="feature")
target_5km_30_sp$target[2:nrow(target_5km_30_sp)]<-target_5km_30_sp$pass_floor[2:nrow(target_5km_30_sp)]
target_5km_30_sp<-target_5km_30_sp[,-5]
###initiate a problem-------------
...
for(i in c(1)){
  ...
  # write.csv(s_grid_cell,"numfo.csv",row.names=F)
