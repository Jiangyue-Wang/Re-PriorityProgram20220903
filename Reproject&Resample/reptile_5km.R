...
reptile<-read.csv("E:/Priority program/Re-PriorityProgram20220903/ReAOH/reptl_spp_list.csv")
REPTILES <- "E:/Priority program/Re-PriorityProgram20220903/ReAOH/REPTILES_narrow"
SPList <- read.csv("E:/Priority program/Re-220327/Input/SPList_32600_220428.csv")
SPList <- SPList[SPList$taxa=="reptile",]

PAKBAlist <- grid_cell[grid_cell$PAorKBA==1,]$X


for(i in 1:length(dir(REPTILES))){
  spraster <- raster(paste0(REPTILES,"/",dir(REPTILES)[i]))
  ...
  name <- reptile[paste0(reptile$filename,".tif")==dir(REPTILES)[i],"binomial"]
  season <- reptile[paste0(reptile$filename,".tif")==dir(REPTILES)[i],"seasonal"]
  write.csv(tlist,paste0("E:/Priority program/Re-PriorityProgram20220903/Reproject&Resample/Species_table/reptile/",name,"_",season,".csv"))
  print(paste(i,name))
  ...
  SPList$sp.file[SPID] <- paste0("E:/Priority program/Re-PriorityProgram20220903/Reproject&Resample/Species_table/reptile/",name,"_",season,".csv")
