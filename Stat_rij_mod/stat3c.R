...
library(data.table)
s_grid_cell <- fread("Output_rij_mod/Global_50_1.csv")
cr<-extract(C3,s_grid_cell[,c("x","y")])
cr<-cr[s_grid_cell$selection!=0]
sum(table(cr)/length(cr))

s_grid_cell <- fread("Output_rij_mod/Global_30_1.csv")
...
sum(s_grid_cell[s_grid_cell$country==149,"selection"])/nrow(s_grid_cell[s_grid_cell$country==149,])
...
selfile<-dir("Output_rij_mod")[1:24]
...
for(i in 1:24){
  s_grid_cell<-fread(paste0("Output_rij_mod/",selfile[i]))
  cr<-extract(C3,s_grid_cell[,c("x","y")])
...
write.csv(stat3c,"Stat_rij_mod/Stat3C1212.csv",row.names=F)
