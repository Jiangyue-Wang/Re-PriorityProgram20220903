...
budget_area_5km_30<-allarea*0.315-P_allarea
...
p_5km_30_sp<-problem(x = x_5km_30 , feature = feature_5km_30_sp, cost_column = "UParea", rij = rij_5km_30_sp) %>%         
  ...
  add_shuffle_portfolio(number_solutions = 1,threads = 28)
...
for(i in c(1)){
  ...
  write.csv(s_grid_cell,paste("Output/solve_cou30_",weights[i],".csv",sep=""),row.names=F)
  rm(p_5km_30_sp_wt)
  rm(s_grid_cell)
  rm(s_5km_30_sp)
  rm(s_plot)
